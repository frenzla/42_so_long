...
/*   Updated: 2024/06/28 18:51:37 by alarose          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	display_nb_moves(t_data *data , int nb_moves)
{
	...
	if (!str)
		return (RET_ERR);
	render_banner(data);
	mlx_string_put(data->mlx, data->mlx_win, 20, (data->map.height + 1) * TILE_SIZE - 12, FONT_COLOR, str);
	free(str);
	return (1);
}
...
int	render_banner(t_data *data)
{
	...
	if (!add_logo(data, &banner))
		return (RET_ERR);
	add_coll_tracker(data, &banner);
